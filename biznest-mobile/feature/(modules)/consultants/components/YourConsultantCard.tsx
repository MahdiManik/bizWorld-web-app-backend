...
interface SuggestionsCardProps {
  consultantData: ConsultantLink;
}

const YourConsultantCard = ({ consultantData }: SuggestionsCardProps) => {
  console.log('consul', consultantData?.consultant?.user_profile);
  return (
    <TouchableOpacity
    ...
    >
      <Image
        source={{ uri: consultantData?.consultant?.user_profile?.image?.url }}
        alt="consultants"
      ...
      <View className="flex-1">
        <View className="flex-row items-start justify-between">
          <View>
            <Text className="font-roboto500 text-base text-subtle">
              {consultantData?.consultant?.fullName}
            </Text>
            <Text className="font-roboto400 text-xs text-description-text">
              {consultantData?.consultant?.user_profile?.professionalHeadline}
            </Text>
            <Text className="font-roboto400 text-xs">
              {consultantData?.consultant?.user_profile?.location}
            </Text>
          </View>
          <View className="flex-row items-center gap-1">
            <AntDesign name="star" size={16} color="#E4A70A" />
            <Text className="font-roboto500 text-sm text-subtle">{5.0}</Text>
        ...
        <View className="my-2 flex-row items-center justify-between">
          <Text className="font-roboto500 text-sm text-black">
            $ {consultantData?.budget}{' '}
            <Text className="text-placeholder">/ hour</Text>
          </Text>
        ...
        <Text className="font-roboto400 text-xs text-black">
          {consultantData?.consultant?.user_profile?.introduction}
        </Text>
        <Button
          title={'Message'}
          onPress={() => {}}
...
export default YourConsultantCard;
