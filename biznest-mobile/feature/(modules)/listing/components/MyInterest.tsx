...
import { useGetMyInterestListing } from '../hooks/useListing';
import FloatingActionButton from '@/components/ui/floating-action-button';
import { router } from 'expo-router';
import InterestListingItem from './InterestListingItem';
...
const MyInterest = () => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage, isLoading } =
    useGetMyInterestListing({
      tab: activeTab,
      search: debouncedSearch,
      page_size: 10,
    });

  const interestListings = useMemo(() => {
    return data?.pages.flatMap((page) => page.data) || [];
  }, [data]);

  const renderListingItem = ({ item }: { item: InterestListing }) => (
    <InterestListingItem interestListing={item} />
  );
